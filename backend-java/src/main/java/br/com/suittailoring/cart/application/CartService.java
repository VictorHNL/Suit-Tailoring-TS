...
public class CartService {
  ...
  public record View(
      UUID token, boolean consumed, List<Item> items, BigDecimal subtotal, String currency) {}

  private final CartRepository carts;
  private final VariantRepository variants;

  public CartService(CartRepository c, VariantRepository v) {
    carts = c;
    variants = v;
  }

  public View create() {
    return view(carts.save(new Cart()));
  }

  @Transactional(readOnly = true)
  public View get(UUID token, UUID customer) {
    Cart c = carts.findById(token).orElseThrow(EntityNotFoundException::new);
    c.checkAccess(customer);
    return view(c);
  }

  public View set(UUID token, UUID variantId, int quantity, UUID customer) {
    Cart c = carts.lock(token).orElseThrow(EntityNotFoundException::new);
    c.checkAccess(customer);
    if (quantity > 0) {
      var v = variants.findById(variantId).orElseThrow(EntityNotFoundException::new);
      if (v.getProduct().getStatus() != Product.Status.PUBLISHED || quantity > v.getStock())
        throw new BusinessException("Produto indisponível ou estoque insuficiente");
    }
    c.setItem(variantId, quantity);
    return view(c);
  }

  private View view(Cart c) {
    List<Item> items =
        c.getItems().entrySet().stream()
