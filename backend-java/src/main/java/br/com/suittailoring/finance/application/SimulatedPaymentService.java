...
public class SimulatedPaymentService {
  ...
  public SimulatedPaymentService(
      OrderRepository o, OrderService s, LedgerRepository l, InvoiceRepository i, AuditService a) {
    orders = o;
    service = s;
    ledger = l;
    invoices = i;
  ...
  public OrderView pay(UUID id) {
    ...
    if (o.getStatus() == PurchaseOrder.Status.PAID
        || o.getStatus() == PurchaseOrder.Status.SHIPPED
        || o.getStatus() == PurchaseOrder.Status.DELIVERED) return OrderView.of(o);
    o.pay();
    ledger.save(new LedgerEntry(id, LedgerEntry.Kind.PAYMENT, o.getTotal(), "Pagamento SIMULADO"));
    invoices.save(new Invoice(id));
    audit.record("PAYMENT_SIMULATED", id);
  ...
  public OrderView refund(UUID id) {
    ...
    o.refund();
    service.release(o);
    ledger.save(new LedgerEntry(id, LedgerEntry.Kind.REFUND, o.getTotal(), "Estorno SIMULADO"));
    invoices.findByOrderId(id).ifPresent(Invoice::cancelPending);
    audit.record("REFUND_SIMULATED", id);
    return OrderView.of(o);
  }
}
