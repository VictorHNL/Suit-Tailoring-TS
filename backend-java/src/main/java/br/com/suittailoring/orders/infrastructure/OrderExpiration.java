...
public class OrderExpiration {
  ...
  public OrderExpiration(OrderRepository o, OrderService s) {
    orders = o;
    service = s;
  }
  ...
  public void expire() {
    for (var order :
        orders.findTop100ByStatusAndExpiresAtBefore(
            PurchaseOrder.Status.AWAITING_PAYMENT, Instant.now())) {
      try {
        service.expire(order.getId());
      } catch (RuntimeException e) {
        org.slf4j.LoggerFactory.getLogger(getClass())
            .warn("Falha ao expirar pedido {}", order.getId());
      }
    }
  }
}
