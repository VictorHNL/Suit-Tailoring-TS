...
public class CrmController {
  ...
  public CrmController(
      AccountRepository a, CustomerNoteRepository n, OrderService o, AuditService log) {
    accounts = a;
    notes = n;
    orders = o;
    audit = log;
  ...
  @GetMapping
  public Page<Profile> customers(@RequestParam(defaultValue = "0") int page) {
    return accounts
        .findByRole(Account.Role.CUSTOMER, PageRequest.of(Math.max(0, page), 20, Sort.by("name")))
        .map(Profile::of);
  }

  @GetMapping("/{id}/orders")
  public Page<OrderView> orders(@PathVariable UUID id, @RequestParam(defaultValue = "0") int page) {
    customer(id);
    return orders.list(id, page);
  }

  @GetMapping("/{id}/notes")
  public Page<CustomerNote> notes(
      @PathVariable UUID id, @RequestParam(defaultValue = "0") int page) {
    customer(id);
    return notes.findByCustomerId(
        id, PageRequest.of(Math.max(0, page), 20, Sort.by("createdAt").descending()));
  }

  @PostMapping("/{id}/notes")
  @Transactional
  public CustomerNote note(@PathVariable UUID id, @Valid @RequestBody Note input, Principal p) {
    customer(id);
    var note = notes.save(new CustomerNote(id, input.content(), p.getName()));
    audit.record("CRM_NOTE_CREATED", id);
    return note;
  }
