...
class LoginIntegrationTest {
    ...
    @Test void customerSessionCannotBecomeAnOwnerSession() throws Exception{
        String email=UUID.randomUUID()+"@example.com";
        accounts.save(new Account("Cliente",email,encoder.encode("password-12345"),Account.Role.CUSTOMER));
        var result=mvc.perform(post("/api/auth/login").with(csrf()).contentType("application/json")
            .content("{\"email\":\""+email+"\",\"password\":\"password-12345\"}")).andExpect(status().isOk()).andReturn();
        var session=(MockHttpSession)result.getRequest().getSession(false);
        mvc.perform(get("/api/auth/me").session(session)).andExpect(status().isOk()).andExpect(jsonPath("$.role").value("CUSTOMER"));
        mvc.perform(get("/api/admin/products").session(session)).andExpect(status().isUnauthorized());
        mvc.perform(post("/api/admin/auth/login").session(session).with(csrf()).contentType("application/json")
            .content("{\"email\":\""+email+"\",\"password\":\"password-12345\"}")).andExpect(status().isUnauthorized());
    }
    @Test void ownerSessionIsSeparateFromCustomerSession() throws Exception{
        String email=UUID.randomUUID()+"@example.com";
