...
class VariantTest {
    @Test void reserveAndReleaseStock() {
        Variant v=new Variant(new Product("Blazer","Descrição"),"SKU-1","M","Preto",3);
        v.reserve(2); assertEquals(1,v.getStock());
        v.release(2); assertEquals(3,v.getStock());
    }
    @Test void insufficientStockDoesNotMutate() {
    ...
    @Test void constructorRejectsBlankProductName() {
