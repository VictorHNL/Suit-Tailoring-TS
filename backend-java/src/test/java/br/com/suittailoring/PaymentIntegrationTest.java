...
class PaymentIntegrationTest {
    ...
    @Test void paymentAndRefundAreIdempotentAndPreserveLedger(){
        var user=accounts.save(new Account("Cliente",UUID.randomUUID()+"@example.com","unused",Account.Role.CUSTOMER));
        var address=addresses.save(new Address(user.getId(),"Rua 1","São Paulo","SP","01001000"));
        var p=new Product("Camisa","Descrição");
        p.update("Camisa","Descrição",UUID.randomUUID().toString(),"Camisas",null,null,Product.Audience.UNISEX,Product.BodyPart.TORSO,new BigDecimal("150.00"),null,List.of("https://example.com/photo.jpg"),null);
        p.publish();products.save(p);
        var variant=variants.save(new Variant(p,UUID.randomUUID().toString(),"M","Branca",2));
        var cart=carts.create();carts.set(cart.token(),variant.getId(),1,null);
        var order=orders.checkout(user.getId(),cart.token(),address.getId(),"payment-test");
        payments.pay(order.id());payments.pay(order.id());
        assertEquals(1,ledger.countByOrderIdAndKind(order.id(),LedgerEntry.Kind.PAYMENT));
        assertEquals(Invoice.Status.PENDING_PROVIDER,invoices.findByOrderId(order.id()).orElseThrow().getStatus());
        payments.refund(order.id());payments.refund(order.id());
        assertEquals(1,ledger.countByOrderIdAndKind(order.id(),LedgerEntry.Kind.REFUND));
        assertEquals(2,variant.getStock());
        assertEquals(Invoice.Status.CANCELLED,invoices.findByOrderId(order.id()).orElseThrow().getStatus());
    }
}
